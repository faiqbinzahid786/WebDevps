...
$querynum = $this->db->query('SELECT *, from_unixtime(adddate,"%d-%m-%Y %H:%i:%s") as adddate
    ...
    WHERE owner_id="'.$_SESSION['owner_id'].'" AND pay_type="4"   AND cus_name LIKE "%'.$data['searchtext'].'%" OR owner_id="'.$_SESSION['owner_id'].'" AND pay_type="4"  AND sale_runno = "'.$data['searchtext'].'"
    ORDER BY ID DESC ');
...
$query = $this->db->query('SELECT *, from_unixtime(sh.adddate,"%d-%m-%Y %H:%i:%s") as adddate
    ...
    WHERE sh.owner_id="'.$_SESSION['owner_id'].'" AND sh.pay_type="4" AND sh.cus_name LIKE "%'.$data['searchtext'].'%" OR sh.owner_id="'.$_SESSION['owner_id'].'" AND sh.pay_type="4" AND sh.sale_runno = "'.$data['searchtext'].'"
...
$query = $this->db->query('UPDATE sale_list_header 
    SET money_from_customer='.$data['money_from_customer'].',
    money_changeto_customer='.$data['money_changeto_customer'].',
